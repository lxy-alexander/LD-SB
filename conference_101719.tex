...
\title{Simplicity Bias in 1-Hidden Layer Neural Networks\\}

\author{\IEEEauthorblockN{1\textsuperscript{st} Alexander Li}
\IEEEauthorblockA{\textit{Binghamton University} \\
xli49@binghamton.edu}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Michael Bronikowski}
\IEEEauthorblockA{\textit{Binghamton University} \\
...
\begin{abstract}
Recent work has shown that neural networks exhibit simplicity bias (SB): they rely on the simplest predictive features even when more robust but complex features exist. Prior demonstrations of SB primarily use semi-synthetic datasets due to the lack of a rigorous feature definition.

Morwani et al. formally define SB for one-hidden-layer neural networks and theoretically prove that, in the infinite-width and linearly separable setting, such networks depend almost exclusively on the 1-dimensional separable subspace, regardless of the presence of many richer features. Empirically, Morwani et al. show that models trained on real datasets such as ImageNet and Waterbirds–Landbirds rely on a low-dimensional projection of the inputs. Finally, Morwani et al. introduce an ensemble method that sequentially removes previously used features to encourage diversity, yielding models with improved robustness to Gaussian noise.
...
Several considerations motivate these hypotheses:
...
To evaluate these hypotheses, we define the following quantitative criteria:
...
The control criterion ensures that observed changes in LD-SB metrics arise from differences in depth rather than discrepancies in training quality or optimization stability.
...
\subsection{OrthoP Baseline on Waterbirds}
\label{sec:orthop_waterbirds}

Before examining how depth affects LD-SB, we first establish that the phenomenon exists on Waterbirds using the OrthoP methodology. We train 1-hidden layer networks in both rich and lazy regimes for 100 epochs and measure LD-SB metrics. Table~\ref{tab:orthop_waterbirds} shows the results.

\begin{table}[htbp]
\centering
\caption{OrthoP results on Waterbirds (1 hidden layer, 100 epochs). Both regimes show strong LD-SB with low $P_{\perp}$-pC.}
\label{tab:orthop_waterbirds}
\begin{tabular}{ccccc}
\toprule
Regime & Val Acc (\%) & Eff. Rank & $P_{\perp}$-pC (\%) & $P$-pC (\%) \\
\midrule
Rich & 86.41 & 8.25 & 0.0 & 36.4 \\
Lazy & 86.41 & 8.52 & 0.1 & 40.0 \\
\bottomrule
\end{tabular}
\end{table}

Both regimes achieve similar final accuracy and effective rank, with strong LD-SB signatures ($P_{\perp}$-pC $\approx 0$). Figure~\ref{fig:orthop_rank} shows the rank evolution during training -- the rich regime collapses faster initially but both converge to similar low ranks.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{results/orthop_waterbirds_rank.png}
\caption{Effective rank evolution during OrthoP training on Waterbirds. Both regimes exhibit rank collapse, converging to $\sim$8 effective dimensions.}
\label{fig:orthop_rank}
\end{figure}

This establishes the baseline: 1-layer networks on Waterbirds exhibit strong LD-SB regardless of training regime. We now investigate how adding depth changes this behavior.
...
Our experiments provide strong empirical support for the hypothesis that \textbf{increasing MLP classifier depth significantly reduces simplicity bias}.

\textbf{Why does depth reduce simplicity bias?}

The dramatic reduction in $P$-pC from 87.1\% to 17.6\% indicates a fundamental change in how the model makes decisions:

\begin{itemize}
    \item \textbf{Shallow networks (1 layer):} For 87\% of mixed samples, the prediction follows only the $P$-subspace contribution. The model essentially ignores $\sim$2034 of the 2048 feature dimensions.
    
    \item \textbf{Deep networks (5+ layers):} For only 17.6\% of samples does the prediction ignore the $P^{\perp}$ contribution. The model integrates information from more dimensions.
\end{itemize}

\textbf{Why does rank$(P)$ increase?}

The increase from 11 to 14 dimensions indicates that deep networks require more independent feature directions to maintain prediction stability. While numerically modest, this represents a 27\% increase in decision complexity. In the LD-SB framework, even small rank changes indicate fundamentally different decision structures.

\textbf{Why does the effect saturate?}

The identical metrics for 5-layer and 10-layer networks suggest that:
\begin{itemize}
    \item The Imagenette classification task has intrinsic complexity requiring $\sim$14 feature dimensions
    \item Beyond 5 layers, additional depth provides no further benefit for this task
...
\subsection{Rich vs Lazy: Why Depth Matters}

Comparing rich and lazy training helps explain \emph{why} depth affects simplicity bias. Figure~\ref{fig:rank_bar} shows the difference.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{results/plot_rank_comparison_bar.png}
\caption{Effective rank in rich vs lazy regimes. Rich regime shows depth-dependent rank increase; lazy regime stays high regardless of depth.}
\label{fig:rank_bar}
\end{figure}

\textbf{The depth effect comes from training dynamics, not architecture.} If it was just about having more layers, both regimes would behave the same. But they don't:

\begin{itemize}
    \item \textbf{Rich regime:} Rank goes from 4.68 (1-layer) to 95.74 (50-layer). More layers = less rank collapse.
    
    \item \textbf{Lazy regime:} Rank stays around 96 no matter how deep. Weights don't change much from initialization so there's nothing to collapse.
\end{itemize}

This tells us the depth effect works through \textbf{gradient-based learning}. In rich training, gradients have to pass through more layers in deeper networks, which spreads out weight updates and prevents any single layer from collapsing too much. Lazy training doesn't have this issue since weights barely move anyway.

\textbf{Convergence at 50 layers.} Interestingly, 50-layer networks end up with similar ranks in both regimes ($\sim$96). Deep enough rich networks start to look like lazy networks -- the updates get spread so thin across layers that the first layer doesn't collapse much.

In practice this means using deeper networks could help avoid extreme simplicity bias when training in rich regime.
